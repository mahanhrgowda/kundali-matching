...
bride_name = st.text_input("Name", "Bride")
bride_date = st.date_input("DOB 📅", value=default_date)
bride_time = st.time_input("TOB ⏰", value=default_time)
bride_tz_list = sorted(list(zoneinfo.available_timezones()))
bride_tz_index = bride_tz_list.index(default_tz)
bride_tz = st.selectbox("Timezone 🌍", options=bride_tz_list, index=bride_tz_index)
bride_lat = st.number_input("Lat 📍", value=default_lat)
bride_lon = st.number_input("Lon 📍", value=default_lon)

st.header("Groom's Details 🤵")
groom_name = st.text_input("Name", "Groom")
groom_date = st.date_input("DOB 📅", value=default_date)
groom_time = st.time_input("TOB ⏰", value=default_time)
groom_tz_list = sorted(list(zoneinfo.available_timezones()))
groom_tz_index = groom_tz_list.index(default_tz)
groom_tz = st.selectbox("Timezone 🌍", options=groom_tz_list, index=groom_tz_index)
groom_lat = st.number_input("Lat 📍", value=default_lat)
groom_lon = st.number_input("Lon 📍", value=default_lon)
